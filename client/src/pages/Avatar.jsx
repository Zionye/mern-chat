...
const Avatar = ({userId, username, online}) => {
  ...
  return (
    <div className={`w-8 h-8 relative rounded-full flex items-center justify-center ${color}`}>
      ...
      {online && (
        <div className="absolute w-3 h-3 bg-green-400 absolute bottom-0 right-0 rounded-full border border-white"></div>
      )}
      {!online && (
        <div className="absolute w-3 h-3 bg-gray-400 absolute bottom-0 right-0 rounded-full border border-white"></div>
      )}
    </div>
  )
}
